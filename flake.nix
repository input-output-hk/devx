    ...
    let
      overlays = {
        ...
        static-libs = (final: prev: {
          ...
          static-gmp =
            (final.gmp.override { withStatic = true; }).overrideDerivation
            (old: {
              configureFlags = old.configureFlags
                ++ [ "--enable-static" "--disable-shared" ];
            });
        ...
        oldR = (final: prev: {
          R_4_1_3 = final.R.overrideDerivation (old: rec {
            version = "4.1.3";
            patches =
              [ ]; # upstream patches will most likely break this build, as they are specific to a different version.
            src = final.fetchurl {
              url = "https://cran.r-project.org/src/base/R-${
                  final.lib.versions.major version
                }/${old.pname}-${version}.tar.gz";
      ...
      let
        ...
        devShellsWithToolsModule = toolsModule:
          ...
          in (builtins.mapAttrs (compiler-nix-name: compiler:
            import ./dynamic.nix {
              inherit pkgs self compiler compiler-nix-name toolsModule;
              withIOG = false;
            }) (compilers pkgs) // pkgs.lib.mapAttrs'
            ...
            (compiler-nix-name: compiler:
              ...
              (import ./static.nix {
                ...
                withIOG = false;
              })) (compilers static-pkgs.buildPackages) // pkgs.lib.mapAttrs'
            (compiler-nix-name: compiler:
            ...
            (compiler-nix-name: compiler:
              ...
              (import ./cross-js.nix {
                pkgs = js-pkgs.buildPackages;
                inherit self compiler compiler-nix-name toolsModule;
              })) (js-compilers js-pkgs.buildPackages) // pkgs.lib.mapAttrs'
            (compiler-nix-name: compiler:
            ...
            (compiler-nix-name: compiler:
              ...
              (import ./cross-windows.nix {
                pkgs = windows-pkgs;
                inherit self compiler compiler-nix-name;
        ...
        devShellsWithEvalOnLinux =
          devShellsWithToolsModule { evalSystem = "x86_64-linux"; };
      in {
        inherit devShells;
        hydraJobs = devShells // {
          ...
          required = pkgs.runCommand "test-dependencies" {
            ...
            constituents = map (name: "${system}.${name}-env")
              (builtins.attrNames devShellsWithEvalOnLinux);
          } "touch  $out";
        } // (pkgs.lib.mapAttrs' (name: drv:
          pkgs.lib.nameValuePair "${name}-env" (let
            env = pkgs.runCommand "${name}-env.sh" {
        ...
        packages.cabalProjectLocal.static = (import ./quirks.nix {
          pkgs = static-pkgs;
          static = true;
        }).template;
        packages.cabalProjectLocal.cross-js =
          (import ./quirks.nix { pkgs = js-pkgs; }).template;
        packages.cabalProjectLocal.cross-windows =
          (import ./quirks.nix { pkgs = windows-pkgs; }).template;
      });
