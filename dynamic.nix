...
let
  ...
  tool = tool-name:
    pkgs.haskell-nix.tool compiler-nix-name tool-name [
...
in pkgs.mkShell {
  ...
  buildInputs = [ wrapped-cabal fixup-nix-deps compiler ] ++ (with pkgs; [
    pkgconfig
    ...
    (with pkgs;
      ...
      (with pkgs; [ libblst libsodium-vrf secp256k1 R_4_1_3 postgresql icu ]));
}
